...
test.describe('Filter nach Bewertung (Story 11)', () => {
  ...
  test('K8: Kombination Bewertung + Kategorie', async ({ page }) => {
    // Given: "Dinkelbrot" (Brot, 5 Sterne), "Roggenbrot" (Brot, 2 Sterne), "Spaghetti Bolognese" (Mittagessen, 5 Sterne)
    const suffix = Date.now();
    await createRecipeWithRating(page, `Dinkelbrot-${suffix}`, ['Brot'], 5);
    await createRecipeWithRating(page, `Roggenbrot-${suffix}`, ['Brot'], 2);
    await createRecipeWithRating(page, `SpaghettiKombi-${suffix}`, ['Mittagessen'], 5);
    ...
    await expect(page.locator('.recipe-item h2', { hasText: `SpaghettiKombi-${suffix}` })).not.toBeVisible();
  ...
  test('K9: Kombination Bewertung + Volltextsuche', async ({ page }) => {
    // Given: "Dinkelbrot" (4 Sterne), "Dinkelpfannkuchen" (1 Stern) — beide enthalten "Dinkel"
    const suffix = Date.now();
    await createRecipeWithRating(page, `Dinkelbrot-${suffix}`, ['Brot'], 4);
    await createRecipeWithRating(page, `Dinkelpfannkuchen-${suffix}`, ['Snacks'], 1);

    // When: Suche nach "Dinkelbrot-{suffix}" + Filter "gut" — direkte URL
    await page.goto(`/?q=Dinkelbrot-${suffix}&bewertung=gut`);

    // Then: "Dinkelbrot" sichtbar (4 Sterne, enthält Suchbegriff)
    await expect(page.locator('.recipe-item h2', { hasText: `Dinkelbrot-${suffix}` })).toBeVisible();

    // And: "Dinkelpfannkuchen" nicht sichtbar (1 Stern, nicht im Suchbegriff enthalten)
  ...
  test('K10: Kombination Bewertung + "Länger nicht gemacht"', async ({ page }) => {
    // Given: "Linseneintopf" (4 Sterne, planned_date 2025-01-01),
    //        "Erbsensuppe" (4 Sterne, planned_date 2026-01-01) — Zukunftsdatum, erscheint nicht
    //        "Tomatensuppe" (2 Sterne, planned_date 2024-01-01)
    const suffix = Date.now();
    await createRecipeWithRating(page, `Linseneintopf-${suffix}`, ['Mittagessen'], 4, '1.1.2025');
    await createRecipeWithRating(page, `Tomatensuppe-${suffix}`, ['Mittagessen'], 2, '1.1.2024');

    // When: URL mit bewertung=gut und filter=laenger-nicht-gemacht
    await page.goto(`/?bewertung=gut&filter=laenger-nicht-gemacht`);

    // Then: "Linseneintopf" erscheint (gute Bewertung, Vergangenheitsdatum)
    ...
    await expect(page.locator('.recipe-item h2', { hasText: `Tomatensuppe-${suffix}` })).not.toBeVisible();
