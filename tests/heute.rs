...
#[tokio::test]
async fn heute_zeigt_rezept_fuer_heute() {
    ...
    // Then: HTTP 200, Rezept im Body sichtbar
    assert_eq!(status, StatusCode::OK);
    assert!(
        body.contains("Spaghetti Bolognese"),
        "Body sollte 'Spaghetti Bolognese' enthalten:\n{}",
        &body[..500.min(body.len())]
    );
}

#[tokio::test]
async fn heute_zeigt_rezept_fuer_gestern() {
    // Given: Rezept mit planned_date = gestern
    let (app, _temp) = setup_test_app().await;
    let gestern_str = date_in_days(-1);
    create_recipe_with_date(&app, "Thai-Curry", &["Mittagessen"], Some(&gestern_str)).await;
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        body.contains("Thai-Curry"),
        "Body sollte 'Thai-Curry' enthalten"
    );
}

#[tokio::test]
async fn heute_zeigt_rezept_fuer_morgen() {
    // Given: Rezept mit planned_date = morgen
    let (app, _temp) = setup_test_app().await;
    let morgen_str = date_in_days(1);
    create_recipe_with_date(&app, "Pfannkuchen", &["Kuchen"], Some(&morgen_str)).await;
    ...
    assert!(
        ...
        "Body sollte 'Pfannkuchen' enthalten"
    );
}

#[tokio::test]
async fn heute_zeigt_keine_rezepte_von_vorgestern() {
    // Given: Rezept mit planned_date = vorgestern
    let (app, _temp) = setup_test_app().await;
    let vorgestern_str = date_in_days(-2);
    create_recipe_with_date(
        &app,
        "Vorgestern-Essen",
        &["Mittagessen"],
    ...
    // When: GET /heute
    let (_, body) = get_body(app, "/heute").await;

    // Then: Rezept NICHT im Body
    assert!(
        !body.contains("Vorgestern-Essen"),
        "Body sollte 'Vorgestern-Essen' NICHT enthalten"
    );
}

#[tokio::test]
async fn heute_zeigt_freundliche_meldung_wenn_kein_rezept_fuer_heute() {
    // Given: Kein Rezept mit planned_date = heute (aber eines für morgen)
    let (app, _temp) = setup_test_app().await;
    let morgen_str = date_in_days(1);
    create_recipe_with_date(&app, "Morgen-Rezept", &["Mittagessen"], Some(&morgen_str)).await;

    // When: GET /heute
    let (status, body) = get_body(app, "/heute").await;

    // Then: HTTP 200, freundliche Meldung sichtbar
    assert_eq!(status, StatusCode::OK);
    assert!(
        body.contains("Für heute noch kein Rezept geplant"),
        "Body sollte freundliche Meldung enthalten:\n{}",
        &body[..500.min(body.len())]
    );
}
...
#[tokio::test]
async fn heute_rating_post_gibt_200_zurueck() {
    // Given: Rezept mit planned_date = heute
    let (app, _temp) = setup_test_app().await;
    let today_str = date_in_days(0);
    let id = create_recipe_with_date(
        &app,
        "Rating-Test-Rezept",
        &["Mittagessen"],
        Some(&today_str),
    ...
    let (status, body) = post_rating(app, id, "4").await;

    // Then: HTTP 200, Body enthält "inline-rating"
    assert_eq!(status, StatusCode::OK);
    assert!(
        body.contains("inline-rating"),
        "Body sollte 'inline-rating' enthalten:\n{}",
        &body[..200.min(body.len())]
    );
}

#[tokio::test]
async fn heute_rating_post_speichert_bewertung() {
    ...
    post_rating(app.clone(), id, "5").await;

    // Then: GET /heute zeigt 5 Sterne für das Rezept (aria-label oder aktiver Stern)
    let (status, body) = get_body(app, "/heute").await;
    assert_eq!(status, StatusCode::OK);
    assert!(
        body.contains("5 von 5 Sternen") || body.contains("5 Sterne") || body.contains("active"),
        "Body sollte die gespeicherte 5-Sterne-Bewertung anzeigen"
    );
}

#[tokio::test]
async fn heute_rating_post_gibt_404_fuer_unbekannte_id() {
    // Given: App ohne entsprechendes Rezept
    let (app, _temp) = setup_test_app().await;

    // When: POST /heute/recipes/99999/rating mit rating=4
    let (status, _) = post_rating(app, 99999, "4").await;
...
#[tokio::test]
async fn heute_rating_post_gibt_400_fuer_ungueltige_bewertung() {
    // Given: Rezept mit planned_date = heute
    let (app, _temp) = setup_test_app().await;
    let today_str = date_in_days(0);
    let id = create_recipe_with_date(
        &app,
        "Ungültige-Bewertung-Rezept",
        &["Mittagessen"],
        Some(&today_str),
    )
    .await;
    ...
    let (status, _) = post_rating(app, id, "6").await;
